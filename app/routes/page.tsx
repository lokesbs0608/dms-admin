...
import { archiveRoutes, getRoutes, unarchiveRoutes } from "../utils/routes";

const Loader = () => {
    const [customers, setCustomers] = useState<IRouteDetails[]>([]);
    const [filteredCustomers, setFilteredCustomers] = useState<IRouteDetails[]>([]);
    const [showModal, setShowModal] = useState(false);
    const [selectedEmployee, setSelectedEmployee] = useState<IRouteDetails | null>(
        null
    );
    ...
    const fetchEmployees = async () => {
        try {
            const response = await getRoutes(); // API endpoint to fetch hubs;
    ...
    useEffect(() => {
        const filtered = customers?.filter((customer) =>
            customer?.route_code?.toLowerCase()?.includes(searchQuery.toLowerCase())
        );
    ...
    const handleDelete = async (id: string) => {
        if (selectedEmployee?.status === "Inactive") {
            try {
                const resp = await unarchiveRoutes(id);
            ...
            try {
                const resp = await archiveRoutes(id);
                toast.success(resp?.message);
                fetchEmployees();
                setShowModal(false); // Close modal after delete
            } catch (error) {
    ...
    const openDeleteModal = (employee: IRouteDetails) => {
                    ...
                    <tbody>
                        {filteredCustomers?.map((customer) => (
                            ...
                            >
                                ...
                                >
                                    {customer?.hub_id?.name}
                                </th>
                                <td className="px-6 py-4">{customer?.route_code}</td>
                                <td className="px-6 py-4 truncate max-w-[20rem]">
                                    {customer?.pincodes?.length > 0
                                        ? customer.pincodes.map((pincode) => (typeof pincode === "string" ? pincode : pincode)).join(", ")
                                        : "NA"}
                                </td>
                                <td className="px-6 py-4 truncate max-w-[20rem]">
                                    {customer?.areas?.length > 0
                                        ? customer.areas.map((area) => (typeof area === "string" ? area : area)).join(", ")
                                        : "NA"}
                                </td>

                                <td className="px-6 py-4">{customer?.from}</td>
                                <td className="px-6 py-4">{customer?.via}</td>
                                <td className="px-6 py-4">{customer?.to}</td>
                                <td className="px-6 py-4">
                                    <Link
                                        href={`routes/${customer?._id}`}
