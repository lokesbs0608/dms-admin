...
import { archiveCustomer, getCustomers, unarchiveCustomer } from "../utils/customer";

const Employees = () => {
    const [customers, setCustomers] = useState<ICustomer[]>([]);
    const [filteredCustomers, setFilteredCustomers] = useState<ICustomer[]>([]);
    const [showModal, setShowModal] = useState(false);
    const [selectedEmployee, setSelectedEmployee] = useState<ICustomer | null>(
        null
    );
    ...
    const fetchEmployees = async () => {
        try {
            const response = await getCustomers(); // API endpoint to fetch hubs;
    ...
    const handleDelete = async (id: string) => {
        if (selectedEmployee?.status === "Inactive") {
            try {
                const resp = await unarchiveCustomer(id);
            ...
            try {
                const resp = await archiveCustomer(id);
                toast.success(resp?.message);
                fetchEmployees();
                setShowModal(false); // Close modal after delete
            } catch (error) {
    ...
    const openDeleteModal = (employee: ICustomer) => {
